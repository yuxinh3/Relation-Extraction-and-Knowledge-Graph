...
class SpacyParser(ParserBackend):
    ...
    def parse(self, sentence, return_doc=False):
        ...
        # Step 1: determine the entities, split into original entities, text, and the modifiers
        entities = list()
        entity_chunks = list()
        for entity in doc.noun_chunks:  # Get all nouns
            # Ignore pronouns such as it,his,her...
            # if entity.root.lemma_ == '-PRON-':  # lemma returns the original version of words: looking-->look, is --> be
            #     continue
            ...
            ent = dict(
                ...
                span_bounds=(entity.start, entity.end),  # EX [0,5) if entity is 'Apple'
                modifiers=[]
            )

            for x in entity.root.children:  # Children: The immediate syntactic dependents of the token.
                if x.dep_ == 'det':  # determiner
                    ent['modifiers'].append({'dep': x.dep_, 'span': x.text, 'lemma_span': x.lemma_})
                elif x.dep_ == 'nummod':  # numeric modifier
                    ent['modifiers'].append({'dep': x.dep_, 'span': x.text, 'lemma_span': x.lemma_})
                elif x.dep_ == 'amod':  # adjectival modifier
                    ent['modifiers'].append({'dep': x.dep_, 'span': x.text, 'lemma_span': x.lemma_})
        ...
        for token in doc:
            ...
            if token.dep_ == 'nsubj':  # nominal subject
                relation_subj[token.head.i] = token.i   # The syntactic parent, or “governor”, of this token; i: The
                # index of the token within the parent document.
            # E.g., A [woman] [playing] the piano...
            elif token.dep_ == 'acl':  # clausal modifier of noun (adjectival clause)
