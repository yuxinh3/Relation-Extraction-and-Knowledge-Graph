...
public class SceneGraph implements java.io.Serializable {
	...
	public String [] getsource() {
		int N = this.relationList().size();
		String [] source = new String[N];
		int i = 0;
		for (SceneGraphRelation edge : this.relationList()) {
			source[i] = String.format("%-20s",edge.getSource());
			i =i +1;
		}
		return source;
	}
	...
	public String [] gettarget() {
		...
		int i = 0;
		for (SceneGraphRelation edge : this.relationList()) {
			target[i] = String.format("%-20s",edge.getTarget());
			i =i +1;
		}
		return target;
	}
	...
	public String [] getrela() {
		int N = this.relationList().size();
		String [] rela = new String[N];
		int i = 0;
		for (SceneGraphRelation edge : this.relationList()) {
			rela[i] = String.format("%-20s",edge.getRelation());
			i =i +1;
		}
		return rela;
	}
	...
	public String [] getattr() {
		...
		for (SceneGraphNode node : this.nodeList()) {
			i = i + 1;
			for (SemanticConcept attribute : node.getAttributes()) {
				i = i + 1;
			}
		}
		int N = i ;
		...
		i = 0;
		for (SceneGraphNode node : this.nodeList()) {
			attr[i] = String.format("node: %-20s", node);
			i = i + 1;
			for (SemanticConcept attribute : node.getAttributes()) {
				attr[i] = String.format("attr: %-20s", attribute);
				i = i + 1;
			}
		}
		return attr;
