...
train_sg_path = root_path + 'data/spice_sg_train.json'  # the json file that includes the parsed triplets
val_sg_path = root_path + 'data/spice_sg_val.json'  # the json file that includes the parsed triplets
spice_sg_folder = root_path + 'data/coco_spice_sg2/'
spice_sg_dict_raw_path = root_path + 'data/spice_sg_dict_raw2.npz'  # the word bag dict created by the parsed triplets in json file
spice_sg_dict_path = root_path + 'data/spice_sg_dict2.npz'

dict_info = json.load(open(root_path+'data/cocobu2.json'))  # original word bag of coco dataset
...
# read the subject, rel, object, and attribute from json file --> nltk.stem.WordNetLemmatizer --> enlarge the word bag --> save to dict
...
else:  # load the saved dict in if-branch and print the stats
	num_ssg = 0
	print('load raw dict:')
